...
class TestBoundaryGeneratorReusability:
    ...
    def test_different_fixtures_use_same_generator(
        self,
        boundary_generator,
        string_boundaries,
        integer_boundaries
...
class TestBoundaryGeneratorEdgeCases:
    ...
    def test_custom_max_length(self, boundary_generator):
        ...
        # 验证包含自定义长度的字符串
        assert any(
            len(bv.value) == custom_length 
            for bv in boundaries 
            if isinstance(bv.value, str)
        )
    
    def test_custom_integer_range(self, boundary_generator):
        """测试自定义整数范围"""
        boundaries = boundary_generator.generate_integer_boundaries(
            min_value=0,
            max_value=100
        )
