...
class TestJWTTokenGeneration:
    ...
    def test_create_access_token_includes_data(self):
        """测试令牌包含所有数据"""
        data = {"sub": "user123", "username": "testuser", "role": "admin", "custom_field": "value"}
        token = create_access_token(data)
    ...
    def test_create_user_token(self):
        """测试创建用户令牌"""
        token = create_user_token(user_id="user123", username="testuser", role="user", password_version=1)
...
class TestJWTTokenVerification:
    ...
    def test_get_user_from_token_valid(self):
        """测试从有效令牌获取用户信息"""
        token = create_user_token(user_id="user123", username="testuser", role="user")
...
class TestTokenSecurity:
    ...
    def test_token_includes_password_version(self):
        """测试令牌包含密码版本"""
        token = create_user_token(user_id="user123", username="testuser", role="user", password_version=5)
    ...
    def test_password_version_invalidation(self):
        """测试密码版本变更使令牌失效"""
        # 创建版本1的令牌
        token_v1 = create_user_token(user_id="user123", username="testuser", role="user", password_version=1)

        # 创建版本2的令牌（密码已更改）
        token_v2 = create_user_token(user_id="user123", username="testuser", role="user", password_version=2)
...
class TestSecurityEdgeCases:
    ...
    def test_token_with_large_payload(self):
        """测试大负载令牌"""
        large_data = {"sub": "user123", "data": "x" * 10000}  # 大量数据
