...
class TestCacheManagerInitialization:
    ...
    def test_init_with_enabled_cache(self):
        ...
        manager = CacheManager(redis_client=mock_redis, key_prefix="test", enabled=True)
    ...
    def test_init_with_disabled_cache(self):
        """测试禁用缓存的初始化"""
        mock_redis = MagicMock()
        manager = CacheManager(redis_client=mock_redis, key_prefix="test", enabled=False)
    ...
    def test_init_without_redis_client(self):
        """测试没有 Redis 客户端的初始化"""
        manager = CacheManager(redis_client=None, key_prefix="test", enabled=True)
...
class TestCacheManagerGetCached:
    ...
    async def test_get_cached_hit_with_pydantic_model(self):
        """测试缓存命中（Pydantic 模型）"""
        mock_redis = AsyncMock()
        mock_redis.get = AsyncMock(return_value='{"id": "123", "username": "alice", "email": "alice@example.com"}')
...
class TestCacheManagerComplexScenarios:
    ...
    async def test_cache_with_pydantic_model_workflow(self):
        ...
        # 模拟从 Redis 获取
        mock_redis.get = AsyncMock(return_value='{"id": "789", "username": "charlie", "email": "charlie@example.com"}')
