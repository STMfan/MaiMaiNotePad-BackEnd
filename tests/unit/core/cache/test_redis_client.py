...
class TestRedisClientConnection:
    ...
    async def test_ensure_connection_success(self):
        ...
        with (
            patch("app.core.cache.redis_client.aioredis.ConnectionPool") as mock_pool_class,
    ...
    async def test_ensure_connection_failure(self):
        ...
        with (
            ...
            mock_redis = AsyncMock()
            mock_redis.ping = AsyncMock(side_effect=RedisConnectionError("连接失败"))
            mock_redis_class.return_value = mock_redis

            # 验证抛出 RedisConnectionError
    ...
    async def test_ping_success(self):
        ...
        with (
            patch.object(client, "_ensure_connection", new_callable=AsyncMock),
    ...
    async def test_ping_failure(self):
        ...
        with patch.object(client, "_ensure_connection", new_callable=AsyncMock) as mock_ensure:
            mock_ensure.side_effect = RedisConnectionError("连接失败")
...
class TestRedisClientBatchOperations:
    ...
    async def test_delete_pattern_success(self):
        ...
        with (
            ...
            # 模拟 SCAN 返回多批键
            mock_client.scan = AsyncMock(side_effect=[(10, ["key1", "key2", "key3"]), (0, ["key4", "key5"])])
...
class TestRedisClientErrorHandling:
    ...
    async def test_timeout_error_handling(self):
        ...
        with (
            ...
            mock_client.get = AsyncMock(side_effect=RedisTimeoutError("操作超时"))
