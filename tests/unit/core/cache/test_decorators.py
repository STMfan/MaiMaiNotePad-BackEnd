...
from app.core.cache.decorators import cached, cache_invalidate, _build_cache_key
...
class TestCachedDecorator:
    ...
    async def test_cached_with_custom_key_builder(self, mock_cache_manager):
        ...
        mock_cache_manager.set_cached.assert_called_once_with(
            "custom:user:123",
            {"id": "123", "name": "Alice"},
            3600
        )
...
class TestBuildCacheKey:
    ...
    def test_build_key_with_multiple_params(self):
        ...
        key = _build_cache_key(
            "kb:list:user:{user_id}:page:{page}",
            get_knowledge_list,
            ("user123", 1),
            {}
        )
    ...
    def test_build_key_skips_self_parameter(self):
        ...
        key = _build_cache_key(
            "user:{user_id}",
            service.get_user,
            (service, "123"),  # args 包含 self 和 user_id
            {}
        )
        assert key == "user:123"
