...
def cache_manager_enabled(mock_redis_client):
    """创建启用缓存的缓存管理器"""
    return CacheManager(redis_client=mock_redis_client, key_prefix="test", enabled=True)


@pytest.fixture
def cache_manager_disabled():
    """创建禁用缓存的缓存管理器"""
    return CacheManager(redis_client=None, key_prefix="test", enabled=False)
...
def app_with_cache(cache_manager_enabled):
    ...
    # 添加缓存中间件
    app.add_middleware(CacheMiddleware, cache_manager=cache_manager_enabled, default_ttl=300)
...
def app_without_cache(cache_manager_disabled):
    ...
    # 添加缓存中间件（但缓存已禁用）
    app.add_middleware(CacheMiddleware, cache_manager=cache_manager_disabled, default_ttl=300)
...
class TestCacheMiddlewareBasics:
    ...
    def test_middleware_initialization(self, cache_manager_enabled):
        """测试中间件初始化"""
        app = FastAPI()
        middleware = CacheMiddleware(app=app, cache_manager=cache_manager_enabled, default_ttl=300)
    ...
    def test_should_cache_get_request(self, cache_manager_enabled):
        """测试 GET 请求应该被缓存"""
        app = FastAPI()
        middleware = CacheMiddleware(app=app, cache_manager=cache_manager_enabled)
    ...
    def test_should_not_cache_excluded_path(self, cache_manager_enabled):
        ...
        app = FastAPI()
        middleware = CacheMiddleware(
            app=app, cache_manager=cache_manager_enabled, excluded_paths=["/admin", "/api/internal"]
        )
...
class TestCacheKeyGeneration:
    ...
    def test_build_cache_key_with_query_params(self, cache_manager_enabled):
        """测试带查询参数的缓存键生成"""
        app = FastAPI()
        middleware = CacheMiddleware(app=app, cache_manager=cache_manager_enabled, cache_query_params=True)
    ...
    def test_build_cache_key_ignore_query_params(self, cache_manager_enabled):
        """测试忽略查询参数的缓存键生成"""
        app = FastAPI()
        middleware = CacheMiddleware(app=app, cache_manager=cache_manager_enabled, cache_query_params=False)
...
class TestCacheStats:
    ...
    def test_get_stats_with_degradation(self, cache_manager_disabled):
        """测试禁用缓存时的统计信息"""
        app = FastAPI()
        middleware = CacheMiddleware(app=app, cache_manager=cache_manager_disabled)
...
class TestCacheMiddlewareIntegration:
    ...
    async def test_cache_hit(self, app_with_cache, mock_redis_client):
        ...
        mock_redis_client.get.return_value = json.dumps(
            json.dumps(cached_data, ensure_ascii=False), ensure_ascii=False
        ).encode()
    ...
    async def test_cache_with_custom_ttl(self, cache_manager_enabled, mock_redis_client):
        ...
        # 添加缓存中间件（自定义 TTL）
        app.add_middleware(CacheMiddleware, cache_manager=cache_manager_enabled, default_ttl=600)  # 10 分钟
    ...
    async def test_excluded_path_not_cached(self, cache_manager_enabled):
        ...
        # 添加缓存中间件（排除 /admin 路径）
        app.add_middleware(CacheMiddleware, cache_manager=cache_manager_enabled, excluded_paths=["/admin"])
...
class TestCacheMiddlewareEdgeCases:
    ...
    async def test_cache_with_no_store_directive(self, cache_manager_enabled, mock_redis_client):
        ...
        app.add_middleware(CacheMiddleware, cache_manager=cache_manager_enabled)
