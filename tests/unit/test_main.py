...
class TestRouteRegistration:
    ...
    def test_websocket_endpoint_registration(self):
        ...
        assert any("{token}" in route for route in ws_routes), "WebSocket endpoint missing token parameter"
...
class TestStaticFileRoutes:
    ...
    def test_path_traversal_protection(self):
        ...
        for path in traversal_attempts:
            ...
            assert response.status_code in [403, 404], f"Path traversal not blocked for: {path}"
    ...
    def test_file_not_found_handling(self):
        ...
        for file_path in non_existent_files:
            ...
            assert response.status_code == 404, f"Should return 404 for non-existent file: {file_path}"
