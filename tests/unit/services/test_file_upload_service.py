...
class TestFileUploadServiceSaveFileMocked:
    ...
    @patch("os.path.exists")
    @patch("app.services.file_upload_service.datetime")
    async def test_save_uploaded_file_with_size_duplicate_mocked(
        self, mock_datetime, mock_exists, mock_makedirs, mock_open
    ):
        """测试保存重复文件名（使用 Mock）
...
class TestFileUploadServiceMetadata:
    ...
    def test_create_metadata_file_success(self):
        ...
        - 文件名包含前缀
        """
        metadata = {"name": "Test KB", "version": "1.0.0", "description": "Test description"}
    ...
    def test_create_metadata_file_with_unicode(self):
        ...
        - Unicode 字符被正确保存
        """
        metadata = {"name": "测试知识库", "description": "包含中文的描述"}
...
class TestFileUploadServiceFileSystemErrors:
    ...
    async def test_upload_knowledge_base_disk_space_error_during_file_save(
        ...
        mock_save_file.side_effect = HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="文件保存失败: No space left on device"
        )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await self.service.upload_knowledge_base(
                files=[mock_file], name="Test KB", description="Test Description", uploader_id="test_user_id"
            )
...
class TestFileUploadServiceMetadataErrors:
    ...
    def test_extract_version_with_multiple_version_fields(self):
        ...
        - 返回第一个找到的版本
        """
        data = {"version": "1.0.0", "schema_version": "2.0.0", "card_version": "3.0.0"}
        result = self.service._extract_version_from_toml(data)
    ...
    def test_create_metadata_file_with_invalid_json(self):
        ...
        metadata = {"name": "Test", "created_at": datetime.datetime.now()}  # Not JSON serializable
    ...
    def test_create_metadata_file_with_none_values(self):
        ...
        - None 值被正确序列化
        """
        metadata = {"name": "Test", "description": None, "version": None}
    ...
    def test_create_metadata_file_with_very_large_data(self):
        ...
        - 大数据被正确处理
        """
        metadata = {"name": "Test", "large_field": "x" * 1000000}  # 1MB of data
    ...
    def test_create_metadata_file_with_unicode_characters(self):
        ...
        - Unicode 字符被正确编码
        """
        metadata = {"name": "测试 Test テスト 🎉", "description": "包含中文、日文和 emoji"}
    ...
    def test_create_metadata_file_with_nested_structures(self):
        ...
        metadata = {
            "name": "Test",
            "config": {"settings": {"option1": True, "option2": [1, 2, 3], "option3": {"nested": "value"}}},
        }
...
class TestFileUploadFilenameValidation:
    ...
    def test_validate_filename_with_unicode(self):
        ...
        - Unicode 字符文件名被正确处理
        """
        unicode_names = ["测试文件.txt", "テスト.txt", "тест.txt", "🎉emoji🎊.txt", "文件名_with_中文.txt"]
    ...
    def test_validate_filename_with_spaces(self):
        ...
        - 空格文件名被正确处理
        """
        filenames_with_spaces = ["test file.txt", " leading_space.txt", "trailing_space .txt", "multiple   spaces.txt"]
...
class TestFileUploadFileExtensionChecking:
    ...
    def test_extension_case_insensitive(self):
        ...
        - 混合大小写扩展名被接受
        """
        case_variations = ["test.txt", "test.TXT", "test.Txt", "test.tXt", "test.JSON", "test.Json"]
    ...
    def test_extension_with_multiple_dots(self):
        ...
        - 只检查最后一个扩展名
        """
        filenames = ["test.backup.txt", "file.v1.0.txt", "archive.tar.gz", "data.2024.01.01.json"]
    ...
    def test_extension_missing(self):
        ...
        - 无扩展名文件被拒绝
        """
        filenames_without_extension = ["noextension", "file_without_ext", "README"]
    ...
    def test_extension_hidden_file(self):
        ...
        - 隐藏文件被正确处理
        """
        hidden_files = [".hidden", ".gitignore", ".env"]
    ...
    def test_extension_disallowed_types(self):
        ...
        - 不允许的类型被拒绝
        """
        disallowed_extensions = [".exe", ".dll", ".bat", ".sh", ".py", ".js", ".php", ".asp", ".jsp"]
