...
class TestModerationServiceInitialization:
    ...
    def test_init_with_custom_base_url(self):
        """测试使用自定义 API 地址"""
        service = ModerationService(api_key="test-api-key", base_url="https://custom.api.com/v1")
...
class TestModerateMethod:
    ...
    def test_moderate_normal_text(self, service, mock_openai_response):
        """测试审核正常文本"""
        result_json = json.dumps({"decision": "true", "confidence": 0.15, "violation_types": []})
    ...
    def test_moderate_violation_text(self, service, mock_openai_response):
        """测试审核违规文本"""
        result_json = json.dumps({"decision": "false", "confidence": 0.92, "violation_types": ["abuse"]})
    ...
    def test_moderate_unknown_text(self, service, mock_openai_response):
        """测试审核不确定文本"""
        result_json = json.dumps({"decision": "unknown", "confidence": 0.65, "violation_types": ["politics"]})
    ...
    def test_moderate_multiple_violations(self, service, mock_openai_response):
        """测试审核包含多种违规类型的文本"""
        result_json = json.dumps({"decision": "false", "confidence": 0.95, "violation_types": ["porn", "abuse"]})
    ...
    def test_moderate_with_text_type(self, service, mock_openai_response):
        """测试指定文本类型"""
        result_json = json.dumps({"decision": "true", "confidence": 0.2, "violation_types": []})
    ...
    def test_moderate_with_custom_parameters(self, service, mock_openai_response):
        """测试使用自定义参数"""
        result_json = json.dumps({"decision": "true", "confidence": 0.1, "violation_types": []})
...
class TestErrorHandling:
    ...
    def test_moderate_invalid_result_format(self, service):
        ...
        invalid_json = json.dumps(
            {"decision": "invalid_value", "confidence": 0.5, "violation_types": []}  # 无效的决策值
        )
...
class TestResultValidation:
    ...
    def test_validate_result_valid(self, service):
        """测试验证有效结果"""
        valid_result = {"decision": "true", "confidence": 0.5, "violation_types": []}

        assert service._validate_result(valid_result) is True

    def test_validate_result_invalid_decision(self, service):
        """测试验证无效的决策值"""
        invalid_result = {"decision": "maybe", "confidence": 0.5, "violation_types": []}  # 无效值
    ...
    def test_validate_result_invalid_confidence(self, service):
        """测试验证无效的置信度"""
        # 置信度超出范围
        invalid_result = {"decision": "true", "confidence": 1.5, "violation_types": []}  # 超过 1.0
    ...
    def test_validate_result_invalid_violation_types(self, service):
        """测试验证无效的违规类型"""
        invalid_result = {"decision": "false", "confidence": 0.9, "violation_types": ["invalid_type"]}  # 无效的违规类型
...
class TestEdgeCases:
    ...
    def test_moderate_very_long_text(self, service):
        ...
        mock_choice = Mock()
        mock_message = Mock()
        mock_message.content = json.dumps({"decision": "true", "confidence": 0.2, "violation_types": []})
        mock_choice.message = mock_message
        mock_response.choices = [mock_choice]
