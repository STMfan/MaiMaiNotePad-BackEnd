...
class TestKnowledgeBaseUpload:
    ...
    async def test_upload_knowledge_base_success(self, mock_datetime, mock_makedirs, mock_db):
        ...
        with patch.object(self.service, "_save_uploaded_file_with_size", new_callable=AsyncMock) as mock_save:
            mock_save.side_effect = [("/path/to/test1.txt", 1024), ("/path/to/test2.json", 2048)]
    ...
    async def test_upload_knowledge_base_too_many_files(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await self.service.upload_knowledge_base(
                files=files, name="Test KB", description="Test", uploader_id="user123"
            )
    ...
    async def test_upload_knowledge_base_invalid_file_type(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await self.service.upload_knowledge_base(
                files=[mock_file], name="Test KB", description="Test", uploader_id="user123"
            )
        ...
        with patch.object(self.service, "_save_uploaded_file_with_size", new_callable=AsyncMock) as mock_save:
            ...
            with patch("os.path.exists", return_value=True):
                ...
                result = await self.service.add_files_to_knowledge_base(
                    kb_id="kb123", files=[mock_file], user_id="user123"
                )
    ...
    async def test_add_files_to_knowledge_base_not_found(self, mock_db):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await self.service.add_files_to_knowledge_base(kb_id="nonexistent", files=[mock_file], user_id="user123")
    ...
    async def test_add_files_to_knowledge_base_too_many_files(self, mock_db):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await self.service.add_files_to_knowledge_base(kb_id="kb123", files=[mock_file], user_id="user123")
    ...
    async def test_delete_files_from_knowledge_base_success(self, mock_remove, mock_exists, mock_db):
        ...
        result = await self.service.delete_files_from_knowledge_base(
            kb_id="kb123", file_id="file123", user_id="user123"
        )
    ...
    async def test_delete_files_from_knowledge_base_kb_not_found(self, mock_db):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await self.service.delete_files_from_knowledge_base(
                kb_id="nonexistent", file_id="file123", user_id="user123"
            )
    ...
    async def test_delete_files_from_knowledge_base_file_not_found(self, mock_db):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await self.service.delete_files_from_knowledge_base(kb_id="kb123", file_id="nonexistent", user_id="user123")
    ...
    async def test_delete_files_from_knowledge_base_remove_failure(self, mock_remove, mock_exists, mock_db):
        ...
        # Execute and verify
        with pytest.raises(HTTPException) as exc_info:
            await self.service.delete_files_from_knowledge_base(kb_id="kb123", file_id="file123", user_id="user123")
...
class TestPersonaCardUpload:
    ...
    async def test_upload_persona_card_wrong_file_count(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await self.service.upload_persona_card(
                files=[], name="Test", description="Test", uploader_id="user123", copyright_owner="Owner"
            )
    ...
    async def test_upload_persona_card_wrong_filename(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await self.service.upload_persona_card(
                files=[mock_file], name="Test", description="Test", uploader_id="user123", copyright_owner="Owner"
            )
    ...
    async def test_upload_persona_card_missing_version(self, mock_rmtree, mock_makedirs, mock_open, mock_toml):
        ...
        assert (
            "未找到版本号字段" in exc_info.value.message
            or "TOML 语法错误" in exc_info.value.message
            or "配置解析失败" in exc_info.value.message
        )
    ...
    async def test_add_files_to_persona_card_success(self, mock_remove, mock_exists, mock_open, mock_toml, mock_db):
        ...
        with patch.object(self.service, "_save_uploaded_file_with_size", new_callable=AsyncMock) as mock_save:
            ...
            # Execute
            result = await self.service.add_files_to_persona_card(pc_id="pc123", files=[mock_file])
    ...
    async def test_add_files_to_persona_card_not_found(self, mock_db):
        ...
        result = await self.service.add_files_to_persona_card(pc_id="nonexistent", files=[mock_file])
    ...
    async def test_add_files_to_persona_card_wrong_file_count(self, mock_db):
        ...
        with pytest.raises(ValidationError) as exc_info:
            await self.service.add_files_to_persona_card(pc_id="pc123", files=[mock_file1, mock_file2])
    ...
    async def test_delete_files_from_persona_card_success(self, mock_remove, mock_exists, mock_db):
        ...
        # Execute
        result = await self.service.delete_files_from_persona_card(pc_id="pc123", file_id="file123", user_id="user123")
    ...
    async def test_delete_files_from_persona_card_not_found(self, mock_db):
        ...
        result = await self.service.delete_files_from_persona_card(
            pc_id="nonexistent", file_id="file123", user_id="user123"
        )
    ...
    async def test_delete_files_from_persona_card_remove_failure(self, mock_remove, mock_exists, mock_db):
        ...
        # Execute and verify
        with pytest.raises(HTTPException) as exc_info:
            await self.service.delete_files_from_persona_card(pc_id="pc123", file_id="file123", user_id="user123")
...
class TestZipCreation:
    ...
    @patch("os.path.exists")
    async def test_create_knowledge_base_zip_success(
        self, mock_exists, mock_zipfile, mock_tempdir, mock_datetime, mock_db
    ):
        """测试成功创建知识库 ZIP 文件
    ...
    @patch("zipfile.ZipFile")
    @patch("os.path.exists")
    async def test_create_persona_card_zip_success(
        self, mock_exists, mock_zipfile, mock_tempdir, mock_datetime, mock_db
    ):
        """测试成功创建人设卡 ZIP 文件
    ...
    @patch("os.path.exists")
    @patch("os.remove")
    async def test_create_zip_error_cleanup(
        self, mock_remove, mock_exists, mock_zipfile, mock_tempdir, mock_datetime, mock_db
    ):
        """测试创建 ZIP 失败时的清理
