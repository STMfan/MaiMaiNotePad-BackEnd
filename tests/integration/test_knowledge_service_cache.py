...
class TestKnowledgeServiceCacheIntegration:
    ...
    def test_get_knowledge_base_by_id_caches_result(
    ...
    def test_update_knowledge_base_invalidates_cache(
        self, test_db: Session, factory: TestDataFactory
    ):
        """测试更新知识库时缓存失效"""
        service = KnowledgeService(test_db)
        ...
        success, msg, updated_kb = service.update_knowledge_base(
            kb.id, update_data, user.id
        )
    ...
    def test_delete_knowledge_base_invalidates_cache(
        self, test_db: Session, factory: TestDataFactory
    ):
        """测试删除知识库时缓存失效"""
        service = KnowledgeService(test_db)
    ...
    def test_get_public_knowledge_bases_caches_result(
        self, test_db: Session, factory: TestDataFactory
    ):
    ...
    def test_get_user_knowledge_bases_caches_result(
        self, test_db: Session, factory: TestDataFactory
    ):
        ...
        # 首次调用（缓存未命中）
        result1, total1 = service.get_user_knowledge_bases(
        ...
        # 第二次调用（应该从缓存获取）
        result2, total2 = service.get_user_knowledge_bases(
            user_id=user.id, page=1, page_size=10
        )
        assert len(result2) == 3
        assert total2 == 3
