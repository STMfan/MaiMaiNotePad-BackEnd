    ...
    def test_server_message_last_message_field_optional(self, client, test_user):
        ...
        with ws_client.connect() as ws:
            ...
            if "last_message" in message:
                ...
                assert last_msg is None or isinstance(last_msg, dict), \
                    "last_message字段应该是None或字典类型"
                
                # 如果是字典，验证包含基本字段
                if isinstance(last_msg, dict) and last_msg:
                    # 消息对象应该有id字段
                    assert "id" in last_msg or len(last_msg) == 0, \
                        "last_message如果不为空，应该包含id字段"
    ...
    def test_server_message_no_extra_required_fields(self, client, test_user):
        ...
        with ws_client.connect() as ws:
            ...
            assert len(unexpected_fields) == 0, \
                f"消息包含未定义的字段: {unexpected_fields}"
