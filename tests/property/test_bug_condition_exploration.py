...
class TestBugConditionUserConflict:
    """
    Bug 条件 1: 用户创建冲突问题
    ...
    **Validates: Requirements 1.1, 1.2**
    ...
    def test_user_creation_no_conflict_after_cleanup(self, test_db):
        ...
        **Validates: Requirements 1.1, 1.2**
        ...
        # 尝试创建用户
        user = service.create_user(username=test_username, email=test_email, password="testpassword123")
...
class TestBugConditionCacheSerialization:
    """
    Bug 条件 2: 缓存序列化问题
    ...
    async def test_cache_can_serialize_user_object(self, test_db, test_user):
        ...
        **Validates: Requirements 1.3, 1.4**
        ...
        try:
            ...
            assert (
                "not json serializable" in error_msg or "serialize" in error_msg
...
class TestBugConditionServiceFailures:
    """
    Bug 条件 3-4: 服务层测试失败
...
class TestBugConditionWebSocket:
    """
...
class TestBugConditionPropertyTests:
    """
    Bug 条件 6: 属性测试失败
    ...
    async def test_boundary_value_handling(self, user_id, test_db):
        ...
        **Validates: Requirements 1.15**
        ...
        try:
            ...
            # 验证返回值类型
            assert result is None or hasattr(result, "id"), "get_user_by_id 应该返回 None 或 User 对象"
...
class TestBugConditionOtherTests:
    """
...
class TestBugConditionTestSuiteFailures:
    """
    综合测试：验证测试套件的整体失败情况
