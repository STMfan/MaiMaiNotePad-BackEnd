    ...
    **Validates: Requirements FR6**
    ...
    def test_concurrent_user_creation_no_duplicates(self, test_db):
        ...
        **Validates: Requirements FR6**
        ...
        assert successful_creations <= 1, (
            f"并发创建相同用户名应该至多只有一个成功，"
            f"实际成功了 {successful_creations} 个"
        )
        ...
        if successful_creations > 0:
            ...
            assert user_count == 1, (
                f"数据库中应该只有一个该用户名的用户，实际有 {user_count} 个"
            )
    ...
    def test_concurrent_persona_creation_maintains_consistency(self, test_db, test_user):
        ...
        **Validates: Requirements FR6**
        ...
        # 验证没有重复的 ID
        assert len(successful_ids) == len(set(successful_ids)), (
            "不应该有重复的 persona ID"
        )
    ...
    def test_concurrent_updates_maintain_consistency(self, num_operations, test_db, test_user):
        ...
        **Validates: Requirements FR6**
        ...
        def update_user(index):
            ...
            try:
                ...
                result = service.update_user(
                    ...
                    username=f"{test_user.username}_updated_{index}"
                )
                session.commit()
                return result is not None
            except Exception as e:
        ...
        if successful_updates > 0:
            assert "updated_" in test_user.username or test_user.username == test_user.username, (
    ...
    **Validates: Requirements FR6**
    ...
    def test_concurrent_transactions_are_isolated(self, test_db, test_user):
        ...
        **Validates: Requirements FR6**
        ...
        try:
            # 更新用户（会自动提交）
            result1 = service1.update_user(
                user_id=test_user.id,
                username=f"{original_username}_updated"
            )
            ...
            try:
                ...
                # 验证会话 2 能看到已提交的更改
                if user_in_session2:
                    assert user_in_session2.username == f"{original_username}_updated", (
                        "会话 2 应该能看到已提交的更改"
                    )
            finally:
                session2.close()
        ...
        # 验证用户的 username 已被更改
        test_db.refresh(test_user)
        assert test_user.username == f"{original_username}_updated", (
            "用户 username 应该被已提交的事务更改"
        )
    ...
    def test_no_orphaned_records_after_operations(self, test_db, test_user):
        ...
        **Validates: Requirements FR6**
        ...
        assert uploader is not None, (
            f"Persona 的上传者 {persona.uploader_id} 应该存在"
        )
