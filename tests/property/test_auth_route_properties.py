    ...
    **Validates: Requirements 3.8**
    ...
    def test_unauthenticated_access_returns_401(
        ...
        **Validates: Requirements 3.8**
        ...
        assert "error" in data or "detail" in data, (
            f"Expected error response for {method} {endpoint}, got: {data}"
        )
        ...
        auth_keywords = ["认证", "授权", "token", "credential", "unauthorized", "authentication", "authenticated", "not authenticated"]
        has_auth_keyword = any(keyword in error_message for keyword in auth_keywords)
        
        assert has_auth_keyword, (
            f"Expected authentication error message for {method} {endpoint}, "
            f"got: {error_message}"
        )
    ...
    @given(
        invalid_token=st.one_of(
            ...
            st.just("Bearer"),
            st.just("Bearer "),
            st.text(alphabet=st.characters(min_codepoint=33, max_codepoint=126), min_size=1, max_size=50).filter(lambda x: not x.startswith("eyJ")),
    ...
    def test_invalid_token_returns_401(
        ...
        **Validates: Requirements 3.8**
        ...
        assert response.status_code == 401, (
            f"Expected 401 for invalid token '{invalid_token[:20]}...', "
        ...
        data = response.json()
        assert "error" in data or "detail" in data, (
            f"Expected error response for invalid token, got: {data}"
        )
    ...
    def test_malformed_bearer_token_returns_401(
        ...
        **Validates: Requirements 3.8**
        ...
        assert response.status_code == 401, (
            f"Expected 401 for malformed bearer '{malformed_bearer[:30]}...', "
            f"got {response.status_code}"
        )
    ...
    def test_token_with_missing_claims_returns_401(self, test_db: Session):
        ...
        **Validates: Requirements 3.8**
        ...
        assert response.status_code == 401, (
            f"Expected 401 for token with missing claims, got {response.status_code}"
        )
        ...
        **Validates: Requirements 3.8**
        ...
        for base_path in base_paths:
            ...
            assert response.status_code in [401, 404, 405, 422], (
                f"Expected 401/404/405/422 for {method} {test_path}, "
                f"got {response.status_code}"
            )
            ...
            if response.status_code == 401:
                data = response.json()
                assert "error" in data or "detail" in data, (
                    f"Expected error response for {method} {test_path}"
                )
