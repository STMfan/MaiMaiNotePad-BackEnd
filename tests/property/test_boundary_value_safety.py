    ...
    **Validates: Requirements FR6**
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            assert result is None or hasattr(result, 'id'), (
                f"get_user_by_id 应该返回 None 或 User 对象，"
                f"实际返回 {type(result)}"
            )
        except (ValueError, TypeError) as e:
            # 预期的异常是可以接受的
            assert "invalid" in str(e).lower() or "type" in str(e).lower() or "none" in str(e).lower(), (
                f"异常消息应该说明问题：{e}"
            )
        except Exception as e:
            # 其他异常不应该发生
            pytest.fail(
                f"get_user_by_id 不应该抛出 {type(e).__name__}: {e}"
            )
    ...
    @given(
        username=st.one_of(
            ...
            st.text(min_size=0, max_size=0),
            st.text(min_size=1, max_size=1000),
            st.text(alphabet=st.characters(min_codepoint=33, max_codepoint=126), min_size=1, max_size=100),  # ASCII 可打印字符
        )
    )
    @settings(max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            assert result is None or hasattr(result, 'username'), (
                f"get_user_by_username 应该返回 None 或 User 对象"
            )
        except (ValueError, TypeError) as e:
            # 预期的异常是可以接受的
            pass
            ...
            # 其他异常不应该发生
            pytest.fail(
                f"get_user_by_username 不应该抛出 {type(e).__name__}: {e}"
            )
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            assert result is None or hasattr(result, 'email'), (
                f"get_user_by_email 应该返回 None 或 User 对象"
            )
        except (ValueError, TypeError) as e:
            # 预期的异常是可以接受的
            pass
            ...
            # 其他异常不应该发生
            pytest.fail(
                f"get_user_by_email 不应该抛出 {type(e).__name__}: {e}"
            )
    ...
    def test_persona_service_handles_boundary_ids(self, persona_id, test_db):
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            # 验证返回值类型正确
            assert result is None or hasattr(result, 'id'), (
                f"get_persona_card_by_id 应该返回 None 或 PersonaCard 对象"
            )
        except (ValueError, TypeError) as e:
            # 预期的异常是可以接受的
            pass
            ...
            pytest.fail(
                f"get_persona_card_by_id 不应该抛出 {type(e).__name__}: {e}"
            )
    ...
    def test_knowledge_service_handles_boundary_ids(self, knowledge_id, test_db):
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            pytest.fail(
                f"get_knowledge_base_by_id 不应该抛出 {type(e).__name__}: {e}"
            )
    ...
    def test_limit_parameter_handles_boundary_values(self, limit, test_db):
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            # 如果 limit 是正数，验证结果数量
            if limit > 0:
                assert len(result) <= limit or limit > 1000, (
                    f"结果数量应该不超过 limit（除非 limit 太大）"
                )
    ...
    **Validates: Requirements FR6**
    ...
    @given(
        description=st.one_of(
            ...
            st.text(alphabet=st.characters(min_codepoint=0x4E00, max_codepoint=0x9FFF), min_size=1, max_size=100),  # 中文
        )
    )
    @settings(max_examples=50, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_description_handles_unicode_characters(self, description, test_db, test_user):
        ...
        **Validates: Requirements FR6**
        ...
        try:
            ...
            pytest.fail(
                f"创建 persona 不应该抛出 {type(e).__name__}: {e}"
            )
