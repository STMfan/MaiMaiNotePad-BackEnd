...
class TestPreservationUserCreation:
    """
    保留属性 1: 用户创建功能保持正常
    ...
    **Validates: Requirements 3.1, 3.4**
    ...
    @given(
        username_suffix=st.text(
            alphabet=st.characters(whitelist_categories=("Ll", "Lu", "Nd")), min_size=4, max_size=12
        ),
        email_prefix=st.text(alphabet=st.characters(whitelist_categories=("Ll", "Nd")), min_size=4, max_size=12),
    )
    @settings(max_examples=20, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_user_creation_with_unique_credentials(self, username_suffix, email_prefix, test_db):
        """
        属性测试：使用唯一凭证创建用户应该成功
        ...
        **Validates: Requirements 3.1, 3.4**
        ...
        # 创建用户
        user = service.create_user(username=username, email=email, password=password)
...
class TestPreservationCacheOperations:
    """
    保留属性 2: 缓存功能保持正常
    ...
    async def test_cache_operations_with_serializable_data(self, cache_value):
        ...
        **Validates: Requirements 3.1, 3.5**
        ...
        if set_result and cache_manager.is_enabled():
            ...
            # 验证缓存的数据与原始数据一致
            assert cached_data == cache_value, f"缓存的数据应该与原始数据一致。原始：{cache_value}，缓存：{cached_data}"
...
class TestPreservationKnowledgeBaseOperations:
    """
    保留属性 3: 知识库功能保持正常
    ...
    **Validates: Requirements 3.1, 3.6**
    ...
    @given(is_public=st.booleans(), description_length=st.integers(min_value=0, max_value=200))
    @settings(max_examples=15, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_knowledge_base_creation_with_various_settings(self, is_public, description_length, test_db, test_user):
        """
        属性测试：知识库创建应该支持各种设置
...
class TestPreservationPersonaOperations:
    """
    保留属性 4: 人设卡功能保持正常
...
class TestPreservationDatabaseOperations:
    """
    保留属性 5: 数据库操作保持正常
    ...
    **Validates: Requirements 3.1, 3.9**
    ...
    async def test_database_transaction_integrity(self, test_db):
        ...
        **Validates: Requirements 3.1, 3.9**
        ...
        user = service.create_user(username=username, email=email, password="TestPassword123!")
    ...
    async def test_service_error_handling_consistency(self, test_db):
        ...
        **Validates: Requirements 3.1, 3.9**
        ...
        try:
            ...
            # 应该返回 None 或抛出预期的异常
            assert result is None or isinstance(result, type(None)), "无效 ID 应该返回 None 或抛出预期异常"
        except (ValueError, TypeError):
            # 这些异常是可以接受的
            pass


class TestPreservationOverallSystemHealth:
    """
    保留属性 7: 系统整体健康状态
