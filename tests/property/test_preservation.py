    ...
    def test_single_test_execution_passes(self):
        ...
        **Validates: Requirements 3.1**
        ...
        assert failed_count == 0, (
            f"Single test execution had {failed_count} failures. " f"This indicates the baseline behavior is broken."
        )
...
class TestFixturePreservation:
    ...
    def test_user_fixtures_create_unique_users(self, test_user, admin_user, test_db):
        ...
        **Validates: Requirements 3.2**
        ...
        assert test_user_email != admin_user_email, "test_user and admin_user should have unique emails"
        assert test_user_username != admin_user_username, "test_user and admin_user should have unique usernames"
...
class TestFactoryPreservation:
    ...
    def test_factory_creates_valid_knowledge_base(self, factory, test_user, test_db):
        ...
        **Validates: Requirements 3.4**
        ...
        kb = factory.create_knowledge_base(uploader=test_user, name="Test KB", description="Test description")
    ...
    def test_factory_creates_valid_user(self, factory):
        ...
        **Validates: Requirements 3.4**
        ...
        user = factory.create_user(username="factoryuser", email="factory@example.com")
    ...
    def test_factory_creates_knowledge_base_file(self, factory, test_user, test_db):
        ...
        **Validates: Requirements 3.4**
        ...
        kb_file = factory.create_knowledge_base_file(knowledge_base=kb, file_name="test.txt", file_path="/tmp/test.txt")
...
class TestHelperPreservation:
    ...
    def test_pytest_markers_work(self):
        ...
        **Validates: Requirements 3.5**
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "--collect-only", "-m", "property", "tests/property/test_preservation.py"],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
class TestPreservationWithPropertyTests:
    ...
    def test_multiple_users_have_unique_identifiers(self, num_users, factory, test_db):
        ...
        **Validates: Requirements 3.2, 3.4**
        ...
        assert len(emails) == len(set(emails)), f"All users should have unique emails, got {emails}"

        # Verify all users have unique usernames
        usernames = [data["username"] for data in user_data]
        assert len(usernames) == len(set(usernames)), f"All users should have unique usernames, got {usernames}"
    ...
    def test_multiple_knowledge_bases_created_correctly(self, factory, test_db):
        ...
        **Validates: Requirements 3.4**
        ...
        for i in range(kb_count):
            kb = factory.create_knowledge_base(uploader=user, name=f"KB {i}", description=f"Description {i}")
            # Capture data immediately
            kb_data.append({"id": kb.id, "uploader_id": kb.uploader_id, "copyright_owner": kb.copyright_owner})
