        ...
        **Validates: Requirements FR6**
        ...
        with patch.object(test_db, "query", side_effect=exception_type("Test error")):
            with caplog.at_level(logging.ERROR):
                ...
                assert result is None, f"get_user_by_id 应该在 {exception_type.__name__} 时返回 None"

                # 验证错误被记录
                assert any(
                    "Error" in record.message or "error" in record.message.lower() for record in caplog.records
        ...
        **Validates: Requirements FR6**
        ...
        with patch.object(test_db, "query", side_effect=exception_type("Test DB error")):
            with caplog.at_level(logging.ERROR):
                ...
                # 验证异常被捕获
                assert result is None, f"get_persona_card_by_id 应该在 {exception_type.__name__} 时返回 None"
        ...
        **Validates: Requirements FR6**
        ...
        with patch.object(test_db, "query", side_effect=exception_type("Test DB error")):
            with caplog.at_level(logging.ERROR):
                ...
                # 验证异常被捕获
                assert result is None, f"get_knowledge_base_by_id 应该在 {exception_type.__name__} 时返回 None"
        ...
        **Validates: Requirements FR6**
        ...
        with patch.object(test_db, "query", side_effect=SQLAlchemyError(error_message)):
            with caplog.at_level(logging.ERROR):
                ...
                assert any("Error" in msg or "error" in msg.lower() for msg in log_messages), "日志应该包含错误信息"
        ...
        for service, method_name, arg in services:
            ...
            with patch.object(test_db, "query", side_effect=SQLAlchemyError("Test error")):
                with caplog.at_level(logging.ERROR):
                    ...
                    # 验证异常被捕获
                    assert result is None, f"{service.__class__.__name__}.{method_name} 应该返回 None"
        ...
        for service, method_name, arg in services_and_methods:
            ...
            with patch.object(test_db, "query", side_effect=SQLAlchemyError("Test error")):
                ...
                try:
                    ...
                    # 验证返回 None 而不是抛出异常
                    assert result is None, f"{service.__class__.__name__}.{method_name} 应该返回 None 而不是抛出异常"
                except SQLAlchemyError:
                    pytest.fail(f"{service.__class__.__name__}.{method_name} 不应该传播 SQLAlchemyError")
        ...
        with patch.object(test_db, "query", side_effect=exception_type("Test error")):
            try:
                ...
                if isinstance(e, SQLAlchemyError):
                    pytest.fail(f"get_user_by_id 不应该传播数据库异常 {exception_type.__name__}")
