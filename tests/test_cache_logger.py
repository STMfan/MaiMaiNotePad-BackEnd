...
class TestCacheLogger:
    ...
    def test_log_cache_get_hit(self, cache_logger, mock_logger):
        """测试记录缓存命中"""
        cache_logger.log_cache_get(key="user:123", hit=True, latency_ms=5.2)
    ...
    def test_log_cache_get_miss(self, cache_logger, mock_logger):
        """测试记录缓存未命中"""
        cache_logger.log_cache_get(key="user:456", hit=False, latency_ms=3.1)
    ...
    def test_log_cache_get_with_error(self, cache_logger, mock_logger):
        ...
        cache_logger.log_cache_get(
            key="user:789", hit=False, latency_ms=10.5, degraded=True, error="Connection refused"
        )
    ...
    def test_log_cache_set_success(self, cache_logger, mock_logger):
        """测试记录缓存设置成功"""
        cache_logger.log_cache_set(key="user:123", success=True, ttl=3600, latency_ms=2.5)
    ...
    def test_log_cache_set_failure(self, cache_logger, mock_logger):
        """测试记录缓存设置失败"""
        cache_logger.log_cache_set(key="user:123", success=False, error="Serialization error")
    ...
    def test_log_cache_invalidate_single_key(self, cache_logger, mock_logger):
        """测试记录单键失效"""
        cache_logger.log_cache_invalidate(key="user:123", success=True)
    ...
    def test_log_cache_invalidate_pattern(self, cache_logger, mock_logger):
        """测试记录批量失效"""
        cache_logger.log_cache_invalidate(pattern="user:*", count=10, success=True)
    ...
    def test_log_cache_disabled(self, cache_logger, mock_logger):
        """测试记录缓存禁用事件"""
        cache_logger.log_cache_disabled(reason="config_enabled_false")
    ...
    def test_json_format_valid(self, cache_logger, mock_logger):
        """测试 JSON 格式有效性"""
        cache_logger.log_cache_get(key="test:key", hit=True, latency_ms=1.0)
