...
class WebSocketTestClient:
    ...
    def __init__(self, test_client: TestClient, token: str, 
                 connection_timeout: float = 5.0, max_retries: int = 3,
                 message_timeout: float = 5.0, enable_message_queue: bool = False):
        """
        初始化WebSocket测试客户端
    ...
    def receive_message(self, timeout: Optional[float] = None, 
                       message_type: MessageType = MessageType.JSON) -> Union[Dict[str, Any], str, bytes, None]:
        """
    ...
    def receive_message_with_validation(self, expected_fields: List[str], 
    ...
    def send_message(self, message: Union[str, Dict[str, Any], bytes], 
    ...
    def simulate_intermittent_connection(self, disconnect_after: float = 1.0, 
                                        reconnect_delay: float = 0.5) -> Dict[str, Any]:
        """
        模拟间歇性连接（连接-断开-重连）
    ...
    def get_exception_simulation_capabilities(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "server_close": ["1000-正常关闭", "1001-端点离开", "1002-协议错误", 
                           "1003-不支持的数据", "1006-异常关闭", "1008-策略违规", 
                           "1011-服务器错误"],
