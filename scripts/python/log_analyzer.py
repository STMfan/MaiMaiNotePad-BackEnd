...
class LogAnalyzer:
    ...
    def analyze_results(self, results: List[Dict]) -> Dict:
        ...
        for result in results:
            ...
            # 提取状态码
            status_codes = re.findall(r'(\d{3})\s+(?:OK|Bad Request|Unauthorized|Forbidden|Not Found|Internal Server Error)', result['content'])
            analysis['status_codes'].update(status_codes)
            
            # 提取请求ID
            request_ids = re.findall(r'ID=([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})', result['content'])
