...
- **上传与行为统计**: 提供上传记录、系统上传统计和下载/收藏行为趋势数据
...
> 以下为后端视角的功能完成情况，用于与前端 README 中的进度说明对齐。

### 认证与用户

- [x] JWT 登录、刷新 Token、注册、找回/重置密码
- [x] 获取当前用户信息、头像上传/删除
- [x] 用户角色管理（admin / moderator / user）

### 知识库模块

- [x] 知识库上传 / 更新 / 删除
- [x] 知识库文件新增、删除与整库打包下载（ZIP）
- [x] 公开知识库查询（分页、搜索、筛选、排序）
- [x] Star / 取消 Star 与 Star 状态查询
- [x] 知识库审核流程（待审核 / 通过 / 拒绝）及相关接口

### 人设卡模块

- [x] 人设卡上传 / 更新 / 删除
- [x] 人设卡文件新增、删除与整卡打包下载（ZIP）
- [x] 公开人设卡查询（分页、搜索、筛选、排序）
- [x] Star / 取消 Star 与 Star 状态查询
- [x] 人设卡审核流程（待审核 / 通过 / 拒绝）及相关接口

### 消息、收藏与统计

- [x] 站内消息发送、列表、标记已读
- [x] 用户个人上传历史与统计接口
- [x] 用户收藏的知识库和人设卡查询接口（`GET /api/user/stars`）
- [x] 管理员上传统计、上传记录、最近用户等基础运营统计接口

### 邮件服务

- [x] 内部验证码邮件发送能力（注册 / 重置密码等）
- [ ] 对外邮件服务 API（文档中列出的 `/api/email/*` 接口）

---

## 迭代阶段规划（Roadmap）

> 以下为后端视角的阶段性规划，主要围绕 API 完整度、性能与稳定性、监控与可观测性、安全与运维能力分阶段演进。

### 阶段一：核心 API 与领域能力（已完成）

**1.1 认证与用户**

- [x] JWT 登录、刷新 Token、注册、找回/重置密码
- [x] 当前用户信息、头像上传/删除、用户角色管理

**1.2 知识库与人设卡**

- [x] 知识库与人设卡的上传 / 更新 / 删除
- [x] 文件新增、删除与整库/整卡 ZIP 打包下载
- [x] 公开列表查询（分页、搜索、筛选、排序）
- [x] Star / 取消 Star / Star 状态查询
- [x] 审核流程（待审核 / 通过 / 拒绝）及审核接口

**1.3 消息与运营统计**

- [x] 站内消息发送、列表、标记已读
- [x] 用户个人上传历史与统计接口
- [x] 管理员上传统计、上传记录、最近用户等基础运营统计
...
- [ ] 关键接口性能基线测试  
  针对登录、上传、审核、下载、消息等核心路径进行基准压测，记录延迟与吞吐指标。
- [ ] 查询与索引优化  
  结合实际数据量与访问模式，为知识库、人设卡、消息等表增加必要索引，优化分页与筛选性能。

**2.2 稳定性与降级策略**

- [ ] 文件上传与下载的超时与重试策略  
  在大文件或网络不稳定场景下，补充合理的超时、重试及错误信息返回。
- [ ] 速率限制与防刷细化  
  已有速率限制器基础上，进一步区分 IP / 用户维度的限流策略，并对异常行为做更精细统计。
...
- [ ] 结构化日志与日志字段规范  
  为关键日志统一字段（request_id、user_id、业务模块等），方便后续接入日志平台。
- [ ] 业务指标统计预留  
  为上传量、审核通过率、错误率等指标预留打点位置，与前端运营看板形成闭环。

**3.2 监控与告警对接预留**

- [ ] 健康检查与探针接口  
  提供 `/health` / `/ready` 等健康检查接口，便于容器编排与负载均衡。
- [ ] 告警通道对接预留  
  为后续接入 Prometheus / Grafana / Sentry 等监控或错误上报系统预留扩展点。
...
- [ ] 权限模型与边界校验回顾  
  逐路由检查角色权限、资源拥有者校验，完善异常路径（例如被封禁用户的行为处理）。
- [ ] 输入验证与防注入  
  对上传内容、搜索参数等进行更严格的字段校验与长度/格式限制。

**4.2 部署与多环境支持**

- [ ] 多环境配置与分层配置管理  
  将开发、测试、生产环境的配置拆分，并提供一致的加载与覆盖策略。
- [ ] 部署脚本与容器化完善  
  完善 Dockerfile / 启动脚本，使部署流程更标准化、可复用。
