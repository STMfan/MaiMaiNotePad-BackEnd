    ...
    try:
        ...
        return {
            ...
            "metrics": {
                "hits_total": sum(
                    sample.value
                    for sample in metrics.cache_hits_total.collect()[0].samples
                ),
                "misses_total": sum(
                    sample.value
                    for sample in metrics.cache_misses_total.collect()[0].samples
                ),
                "degradation_total": sum(
                    sample.value
                    for sample in metrics.cache_degradation_total.collect()[0].samples
                ),
