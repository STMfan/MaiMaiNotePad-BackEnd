...
class AuthService:
    ...
    def create_tokens(self, user: User) -> dict[str, Any]:
        ...
        try:
            ...
            access_token = create_user_token(
                user_id=user.id, username=user.username, role=role, password_version=user.password_version or 0
            )
    ...
    def check_email_rate_limit(self, email: str) -> bool:
        ...
        try:
            ...
            minute_count = (
                self.db.query(EmailVerification)
                .filter(EmailVerification.email == email, EmailVerification.created_at > one_minute_ago)
                .count()
            )
    ...
    def _increment_failed_login(self, user: User) -> None:
        ...
        try:
            ...
            if user.failed_login_attempts >= max_attempts:
                ...
                logger.warning(
                    f"账户因多次登录失败被锁定: " f"username={user.username}, attempts={user.failed_login_attempts}"
                )
