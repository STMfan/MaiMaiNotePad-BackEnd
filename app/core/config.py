...
class Settings(BaseSettings):
    ...
    # Pydantic V2 配置
    model_config = SettingsConfigDict(env_file=".env", case_sensitive=True, extra="allow")  # 允许额外字段以提高灵活性
    ...
    JWT_ACCESS_TOKEN_EXPIRE_MINUTES: int = config_manager.get_int(
        "jwt.access_token_expire_minutes", 15, env_var="JWT_ACCESS_TOKEN_EXPIRE_MINUTES"
    )
    JWT_REFRESH_TOKEN_EXPIRE_DAYS: int = config_manager.get_int(
        "jwt.refresh_token_expire_days", 7, env_var="JWT_REFRESH_TOKEN_EXPIRE_DAYS"
    )
    ...
    # 安全配置
    BCRYPT_ROUNDS: int = config_manager.get_int("security.bcrypt_rounds", 12, env_var="BCRYPT_ROUNDS")
