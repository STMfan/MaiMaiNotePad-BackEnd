...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        if not cache_manager.is_enabled():
            ...
            logger.info(
                f"缓存中间件已初始化: default_ttl={default_ttl}s, "
                f"cache_query_params={cache_query_params}"
            )
    ...
    def _record_degradation(self, reason: str) -> None:
        ...
        logger.warning(
            f"缓存降级: reason={reason}, "
            f"total_degraded={self._stats['degraded']}"
        )
    ...
    def _get_ttl_from_response(self, response: Response) -> Optional[int]:
        """从响应头中提取 TTL
        ...
        try:
            ...
            if cached_data is not None:
                ...
                try:
                    ...
                    # 检查 ETag（如果客户端提供了 If-None-Match）
            ...
            try:
                ...
                if ttl is not None and ttl > 0:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"响应已缓存: {request.url.path}, "
                            f"ttl={ttl}s, size={len(response_body)} bytes"
                        )
                    except Exception as e:
                        logger.warning(f"缓存写入失败 (key={cache_key}): {e}")
                        self._stats["errors"] += 1
                
                # 重新构建响应（因为 body_iterator 已被消费）
                response = Response(
