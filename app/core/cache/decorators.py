...
def cached(
    ...
    Example:
        ...
        # 使用自定义键构建器
        def build_user_key(*args, **kwargs):
            user_id = kwargs.get('user_id') or args[0]
            return f"user:{user_id}"
            
        @cached(key_pattern="user:{user_id}", key_builder=build_user_key)
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if is_async:
            ...
            def sync_wrapper(*args, **kwargs) -> Any:
                # 同步函数不支持缓存（因为 Redis 操作是异步的）
                logger.warning(
                    f"@cached 装饰器不支持同步函数 ({func.__name__})，"
                    "请将函数改为异步函数或直接调用"
                )
...
def cache_invalidate(key_pattern: str, key_builder: Optional[Callable] = None):
    """缓存失效装饰器
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if is_async:
            ...
            def sync_wrapper(*args, **kwargs) -> Any:
                ...
                logger.warning(
                    f"@cache_invalidate 装饰器不支持同步函数 ({func.__name__})，"
                    "请将函数改为异步函数"
                )
                return func(*args, **kwargs)
...
def _build_cache_key(
    ...
    # 需要跳过 args 的第一个元素
    is_bound_method = (
        hasattr(func, '__self__') or  # 绑定方法有 __self__ 属性
        (len(args) > 0 and len(param_names) > 0 and 
         len(args) > len(param_names))  # args 比参数多（可能包含 self）
    )
