...
23. ✅ **知识库/人设卡扩展字段入库**
    - 上传接口新增 `content`、`tags` 字段并落库，兼容旧 metadata 文件
    - 响应新增作者信息、文件列表与大小聚合
    - 数据库迁移新增 content/tags 列并增加默认值处理

24. ✅ **用户上传记录分页筛选增强**
    - `GET /api/knowledge/user/{user_id}` 支持分页、名称/标签/状态筛选与排序
    - `GET /api/persona/user/{user_id}` 支持分页、名称/标签/状态筛选与排序
    - 管理员/审核员可查看他人上传记录

25. ✅ **收藏记录分页与筛选**
    - `GET /api/user/stars` 支持分页、类型过滤（knowledge/persona/all）
    - 支持按创建时间或收藏数排序，返回 `items/total/page/page_size`
    - 参数校验与异常信息优化

26. ✅ **内容删除联动与 CORS 细化**
    - 删除知识库文件时若为最后一个文件，自动删除整个知识库及上传记录
    - 文件删除接口返回 `knowledge_deleted` 标识
    - CORS 暴露 `Content-Disposition` 便于前端获取下载文件名
